...
class Parser():
    ...
    def statement(self):
        ...
        if self.match(TokenType.USE):
            string = self.consume("Need a filename to import",
                                  TokenType.STRING)
    ...
    def identifier(self):
        identifier = self.consume("Expected an identifier",
    ...
    def try_primary(self):
        ...
        if self.match(TokenType.COLON):
            ...
            while self.match(TokenType.RIGHT_ARROW):
                t = self.consume("Expected a type after ->",
                                 TokenType.TYPENAME)
                types.append(t.lexeme)
        if value:
    ...
    def try_primary_(self):
        if self.match(TokenType.OPEN_BRACKETS):
            argument_list = []
            head_arg = self.consume("Argument list cannot be empty",
            ...
            while not self.check(TokenType.CLOSE_BRACKETS):
                e = self.statement()
                body.append(e)
            self.consume("Unclosed function definition",
                         TokenType.CLOSE_BRACKETS)
            return expr.Function(argument_list, body)
        elif self.match(TokenType.STRING, TokenType.B10_FLOAT,
                        TokenType.B10_INTEGER):
            ...
            e = self.symchain()
            self.consume("Missing closing parentheses",
            ...
            while True:
                ...
                self.consume("Missing condition delimiter",
